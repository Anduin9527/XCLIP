...
def validate(val_loader, text_labels, model, config):
    ...
    acc2_meter, acc3_meter = AverageMeter(), AverageMeter()
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            ##TODO 
            values_2,indices_2  = tot_similarity.topk(2,dim=-1)
            ...
            for i in range(b):
                ...
                if label_id[i] in indices_5[i]:
                    acc5 += 1
                if label_id[i] in indices_2[i]:
                    acc2 += 1
                if label_id[i] in indices_3[i]:
                    acc3 += 1
           
            acc1_meter.update(float(acc1) / b * 100, b)
            acc2_meter.update(float(acc2) / b * 100, b)
    ...
    acc5_meter.sync()
    logger.info(f"""*
Acc@1 {acc1_meter.avg:.3f}
Acc@2 {acc2_meter.avg:.3f}
Acc@3 {acc3_meter.avg:.3f}
Acc@5 {acc5_meter.avg:.3f}
""")
